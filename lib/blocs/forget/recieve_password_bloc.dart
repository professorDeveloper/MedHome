...
class RecievePasswordBloc
    extends Bloc<RecievePasswordInitEvent, RecievePasswordState> {
  ...
  RecievePasswordBloc() : super(RecievePasswordInitial()) {
    on<RecievePasswordEvent>((event, emit) async {
      ...
      try {
        ...
        final response = await authApi.recievePassword(
            recievePasswordRequest: event.request);
